    ...
    try {
        for (let iteracao = 1; iteracao <= totalIteracoes; iteracao++) {
            ...
            if (matches.length > 0) {
                ...
                const urlOccurrences = new Map(); // Map para contar ocorrências de cada URL
                ...
                while (!secUserId && retries < maxRetries) {
                    ...
                    try {
                        ...
                        if (urlOccurrences.get(urlDir) >= 15) {
                            console.log(`A URL ${urlDir} foi retornada 15 vezes. Confirmando a ação...`);
