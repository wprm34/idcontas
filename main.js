        ...
        for (let iteracao = 1; iteracao <= totalIteracoes; iteracao++) {
            ...
            if (matches.length > 0) {
                ...
                while (!secUserId && retries < maxRetries) {
                    ...
                    currentUrl = apiResponse.data?.url_dir;
    
                    if (!currentUrl) {
                        console.error('Erro: URL não obtida da API get_action.');
                        break;
                    }
                    
                    try {
                        console.log(`Acessando perfil do TikTok: ${currentUrl}`);
                        await page.goto(currentUrl, { timeout: 999999, waitUntil: 'domcontentloaded' }); // Timeout de 60 segundos
                    } catch (error) {
                        console.error(`Erro ao acessar a URL ${currentUrl}:`, error);
                        continue; // Pule para a próxima iteração se aplicável
                    }                    
...
try {
    ...
    console.error('Erro ao acessar a URL da API:', error);
                ...
                if (uniqueIds.length > 0) {
                        ...
                        try {
                            ...
                            if (actionApiResponse.data.status === 'ENCONTRADA' && actionApiResponse.data.url_dir) {
                                ...
                                if (apiUrl === currentUrl) {
                                    console.log(`URL encontrada: ${apiUrl}`);
                                    foundUrl = true;
                    ...
                    console.log(`URL mais repetida: ${mostFrequentUrl}`);

                    if (mostFrequentUrl) {
                        currentUrl = mostFrequentUrl; // Atualizar para a próxima iteração
